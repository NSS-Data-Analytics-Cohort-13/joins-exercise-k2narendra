...
SELECT count(s.film_title), max(r.imdb_rating)
FROM distributors as d
LEFT JOIN specs as s
ON d.distributor_id = s.domestic_distributor_id
	INNER JOIN rating as r
...
select 
    avg(case when s.length_in_min <= 120 then r.imdb_rating else 0 end) as under_2hr, 
    avg(case when s.length_in_min > 120 then r.imdb_rating else 0 end) as over_2hr
from
    rating as r
join specs as s 
on s.movie_id = r.movie_id;
